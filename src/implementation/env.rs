...
/// See [libmdbx_native::EnvironmentBuilder::set_handle_slow_readers] for more
/// information.
#[cfg(not(windows))]
                ...
                |process_id: u32, thread_id: u32, read_txn_id: u64, gap: usize, space: usize, retry: isize| {
                    if space > MAX_SAFE_READER_SPACE {
                        let message = if is_current_process(process_id) {
                            ...
                            "External process has a long-lived database transaction that grows the database file. Use shorter-lived read \
                             transactions or shut down the node."
                        };
                        tracing::warn!(

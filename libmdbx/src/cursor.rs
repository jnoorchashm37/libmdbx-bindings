...
extern crate ffi;
use ffi::{
    MDBX_cursor_op, MDBX_FIRST, MDBX_FIRST_DUP, MDBX_GET_BOTH, MDBX_GET_BOTH_RANGE, MDBX_GET_CURRENT, MDBX_GET_MULTIPLE, MDBX_LAST,
    MDBX_LAST_DUP, MDBX_NEXT, MDBX_NEXT_DUP, MDBX_NEXT_MULTIPLE, MDBX_NEXT_NODUP, MDBX_PREV, MDBX_PREV_DUP, MDBX_PREV_MULTIPLE,
    ...
    K: TransactionKind
{
    pub(crate) fn new(txn: Transaction<K>, dbi: ffi::MDBX_dbi) -> Result<Self> {
        ...
        unsafe {
            txn.txn_execute(|txn_ptr| mdbx_result(ffi::mdbx_cursor_open(txn_ptr, dbi, &mut cursor)))??;
        }
        Ok(Self { txn, cursor })
    }
    ...
    fn get<Key, Value>(&self, key: Option<&[u8]>, data: Option<&[u8]>, op: MDBX_cursor_op) -> Result<(Option<Key>, Value, bool)>
    where
        Key: TableObject,
        Value: TableObject
    {
    ...
    fn get_value<Value>(&mut self, key: Option<&[u8]>, data: Option<&[u8]>, op: MDBX_cursor_op) -> Result<Option<Value>>
    where
        Value: TableObject
    {
    ...
    fn get_full<Key, Value>(&mut self, key: Option<&[u8]>, data: Option<&[u8]>, op: MDBX_cursor_op) -> Result<Option<(Key, Value)>>
    where
        Key: TableObject,
        Value: TableObject
    {
...
impl Cursor<RW> {
    /// Puts a key/data pair into libmdbx. The cursor will be positioned at
    /// the new data item, or on failure usually near it.
    pub fn put(&mut self, key: &[u8], data: &[u8], flags: WriteFlags) -> Result<()> {
        let key_val: ffi::MDBX_val = ffi::MDBX_val { iov_len: key.len(), iov_base: key.as_ptr() as *mut c_void };
        let mut data_val: ffi::MDBX_val = ffi::MDBX_val { iov_len: data.len(), iov_base: data.as_ptr() as *mut c_void };
        mdbx_result(unsafe {
            self.txn
                .txn_execute(|_| ffi::mdbx_cursor_put(self.cursor, &key_val, &mut data_val, flags.bits()))?
        })?;
...
unsafe fn slice_to_val(slice: Option<&[u8]>) -> ffi::MDBX_val {
    match slice {
        Some(slice) => ffi::MDBX_val { iov_len: slice.len(), iov_base: slice.as_ptr() as *mut c_void },
...
{
    /// Creates a new iterator backed by the given cursor.
    fn new(cursor: &'cur mut Cursor<K>, op: ffi::MDBX_cursor_op, next_op: ffi::MDBX_cursor_op) -> Self {
        ...
        match self {
            IterDup::Ok { cursor, op, .. } => {
                ...
                let result = cursor.txn.txn_execute(|_| {
                    ...
                    (err_code == ffi::MDBX_SUCCESS)
                        .then(|| IntoIter::new(Cursor::new_at_position(&**cursor).unwrap(), ffi::MDBX_GET_CURRENT, ffi::MDBX_NEXT_DUP))
                });

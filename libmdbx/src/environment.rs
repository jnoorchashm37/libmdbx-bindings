...
impl Environment {
    ...
    pub fn begin_rw_txn(&self) -> Result<Transaction<RW>> {
        ...
        let txn = loop {
            ...
            if matches!(&res, Err(Error::Busy)) {
                if !warned {
                    warned = true;
                    warn!(target: "data-server::libmdbx-writer", "Process stalled, awaiting read-write transaction lock");
                }
    ...
    pub fn stat(&self) -> Result<Stat> {
        unsafe {
            ...
            mdbx_result(ffi::mdbx_env_stat_ex(self.env_ptr(), ptr::null(), stat.mdb_stat(), size_of::<Stat>()))?;
            Ok(stat)
        }
    }
    ...
    pub fn info(&self) -> Result<Info> {
        unsafe {
            ...
            mdbx_result(ffi::mdbx_env_info_ex(self.env_ptr(), ptr::null(), &mut info.0, size_of::<Info>()))?;
            Ok(info)
        }
    }
...
pub type HandleSlowReadersCallback =
    fn(process_id: u32, thread_id: u32, read_txn_id: u64, gap: usize, space: usize, retry: isize) -> HandleSlowReadersReturnCode;
...
impl EnvironmentBuilder {
    ...
    pub fn open_with_permissions(&self, path: &Path, mode: ffi::mdbx_mode_t) -> Result<Environment> {
        ...
        unsafe {
            ...
            if let Err(e) = (|| {
                ...
                if let Some(max_readers) = self.max_readers {
                    mdbx_result(ffi::mdbx_env_set_option(env, ffi::MDBX_opt_max_readers, max_readers))?;
                }

                #[cfg(not(windows))]
                if let Some(handle_slow_readers) = self.handle_slow_readers {
                    mdbx_result(ffi::mdbx_env_set_hsr(env, handle_slow_readers_callback(handle_slow_readers)))?;
                }
                ...
                mdbx_result(ffi::mdbx_env_open(env, path.as_ptr(), self.flags.make_flags() | self.kind.extra_flags(), mode))?;
        ...
        let txn_manager = {
            if let crate::MaxReadTransactionDuration::Set(duration) = self
                ...
                .unwrap_or(read_transactions::MaxReadTransactionDuration::Set(DEFAULT_MAX_READ_TRANSACTION_DURATION))
            {
    ...
    pub fn set_geometry<R: RangeBounds<usize>>(&mut self, geometry: Geometry<R>) -> &mut Self {
        ...
        self.geometry = Some(Geometry {
            size:             geometry
                .size
                .map(|range| (convert_bound(range.start_bound()), convert_bound(range.end_bound()))),
...
unsafe fn handle_slow_readers_callback(callback: HandleSlowReadersCallback) -> ffi::MDBX_hsr_func {
    ...
    let hsr = Box::leak(Box::new(
        |_env: *const ffi::MDBX_env,
         ...
         space: usize,
         retry: ::libc::c_int|
         -> i32 { callback(pid as u32, tid as u32, laggard, gap as usize, space, retry as isize).into() }
    ));
